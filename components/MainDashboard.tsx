...
const MainDashboard: React.FC<MainDashboardProps> = ({ benchmarks }) => {
  ...
  const [scenarioMetrics, setScenarioMetrics] = useState<ScenarioMetrics | null>(
    null
  );
  ...
  // ---- CORE LOGIC: forecast and required run rate ----
  const baseMetrics = useMemo(() => {
    ...
    const nrrFactor = actuals.includeCustomerSuccess ? benchmarks.nrr : 1;

    // Incremental ARR from new business + NRR over the target period
    const incrementalArr =
      currentRunRate * monthsInTargetPeriod * nrrFactor;

    // Forecast = current ARR + incremental ARR (so it can’t go below current ARR)
    const forecastArr = benchmarks.currentArr + incrementalArr;

    const gapToTarget = forecastArr - benchmarks.targetArr;

    // Required run rate so that: currentArr + (runRate * months * nrrFactor) = targetArr
    const requiredRunRate =
      monthsInTargetPeriod > 0 && nrrFactor > 0
        ? (benchmarks.targetArr - benchmarks.currentArr) /
          (monthsInTargetPeriod * nrrFactor)
        : 0;
  ...
  const monthsInTargetPeriod = baseMetrics.monthsInTargetPeriod;
  ...
  const funnelBenchmarkComparisons = useMemo(() => {
    ...
    const leadsToMqlExpected = actuals.leads * benchmarks.leadsToMql;
    items.push({
      key: "leadsToMql",
      label: "Leads → MQLs",
      actual: actuals.mqls,
    ...
    const mqlToSqlExpected = actuals.mqls * benchmarks.mqlToSql;
    items.push({
      key: "mqlToSql",
      label: "MQL → SQLs",
      actual: actuals.sqls,
    ...
    const sqlToOppExpected = actuals.sqls * benchmarks.sqlToOpp;
    items.push({
      key: "sqlToOpp",
      label: "SQL → Opps",
      actual: actuals.opps,
    ...
    const oppToProposalExpected = actuals.opps * benchmarks.oppToProposal;
    items.push({
      key: "oppToProposal",
      label: "Opp → Proposals",
  ...
  const computeScenarioMetricsFromNewArr = (
    ...
    const currentRunRate =
      monthsInPeriod > 0 ? scenarioNewArr / monthsInPeriod : 0;
  ...
  const applyScenario = (scenario: ScenarioId) => {
    ...
    if (scenario === "weakest-stage") {
      ...
      if (leadVolumeIsUnderBenchmark) {
        ...
        const mqls = targetLeads * conversionRates.leadToMql;
        ...
        const metrics = computeScenarioMetricsFromNewArr(scenarioNewArr);
  ...
  const handleActualChange = (field: keyof Actuals, value: string) => {
    ...
    setActuals((prev) => {
      ...
      if (field === "newArr") {
        ...
        return {
          ...prev,
          newArr: Number.isFinite(numeric) ? numeric : prev.newArr,
        };
      }
  ...
  const avgAcvDisplay =
    actuals.wins > 0 ? formatCurrency(baseAcv) : "—";
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 shadow-lg shadow-slate-950/40">
        ...
        <div className="mt-1 grid gap-4 md:grid-cols-6">
          ...
          <div className="md:col-span-1 flex items-center justify-center text-sm font-semibold">
            {(() => {
              ...
              const arrow = isAbove ? "↑" : "↓";
              const absDiff = Math.round(Math.abs(leadsDiff));
              if (absDiff === 0) {
          ...
          {funnelBenchmarkComparisons.map((metric) => {
            const isAbove =
              metric.diff !== undefined && metric.diff >= 0;
            const arrow = isAbove ? "↑" : "↓";
            const absDiff = Math.round(Math.abs(metric.diff ?? 0));
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-1 rounded-md border border-slate-700 bg-slate-950 px-2 py-1 text-sm">
              ...
              <input
                ...
                value={
                  actuals.newArr ? formatInteger(actuals.newArr) : ""
                }
                onChange={(e) =>
            ...
            <div className="mt-2">
              ...
              >
                {arrStatusIsAbove ? "Above Target" : "Behind Target"}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            {actuals.wins > 0 && Math.abs(acvDiff) >= 1 && (
              <div className="mt-1 text-sm font-semibold">
                <span
                  className={
                    acvDiff >= 0 ? "text-emerald-400" : "text-red-400"
                  }
                >
                  {acvDiff >= 0 ? "↑ " : "↓ "}
          ...
          <div>
            ...
            >
              <option value="true">Yes, include NRR uplift</option>
              <option value="false">No, new ARR only</option>
            </select>
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Fix lead volume, weak funnel stages, or ACV gaps to
              understand the fastest route to target.
            </p>
          </div>
          {activeScenario && (
            <button
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {/* Scenario 1: Primary fix – LEADS FIRST if badly under benchmark */}
          <div className="flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-400">
                {leadVolumeIsUnderBenchmark ? (
                  <>
                    Lead volume is currently{" "}
                    <span className="font-semibold text-slate-100">
                      {formatInteger(actuals.leads)}
                    </span>{" "}
                    vs a benchmark of{" "}
                    <span className="font-semibold text-slate-100">
                      {formatInteger(Math.round(leadsExpected))}
                    </span>{" "}
                    for this period. See the impact of bringing leads
                    back to benchmark on ARR.
                  </>
                ) : weakestStage ? (
                  <>
                    {weakestStage.label} is currently lagging target.
                    Actual {formatPercent(weakestStage.actual)} vs
                    target {formatPercent(weakestStage.target)}. See
                    the impact of bringing it back to benchmark on ARR.
                  </>
                ) : (
          ...
          <div className="flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-400">
                {acvIsBelowBenchmark ? (
                  <>
                    Current ACV is {formatCurrency(baseAcv)} vs
                    benchmark {formatCurrency(benchmarks.acv)}. See
                    the impact of bringing ACV back to benchmark on
                    ARR.
                  </>
                ) : (
                  <>
                    Current ACV is around {formatCurrency(baseAcv)}. See
                    what happens if you improve pricing, discount
